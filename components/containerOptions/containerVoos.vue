...
onMounted(() => {
  PhotoService.getImagesByCategory('voos').then((data) => (images.value = data));
});
</script>
              ...
              <div class="flex items-center justify-end">
                <div class="grid grid grid-cols-2 gap-x-3">
                  <div class="relative">
                      ...
                      <InputText v-model="a" placeholder="Viajantes" class="md:w-[14rem] h-[43px] text-base items-center"/>
                  </div>
                  <div class="">
                    <Select v-model="selectedCity" :options="cities" optionLabel="name" placeholder="Classes"
          ...
          <template #footer>
            <div class="flex gap-x-5 ">
              <div class="flex flex-col">
                <span class="text-xs uppercase mb-2" :style="{ color: themeMode.darkMode ? 'white' : '#5e685e' }">Origem:</span>
              ...
              <div class="flex flex-col">
                <span class=" text-xs uppercase mb-2" :style="{ color: themeMode.darkMode ? 'white' : '#5e685e' }">Destino:</span>
                <Select v-model="selectedCity" :options="cities" optionLabel="name" placeholder="Selecione uma cidade"
                  class="w-full md:w-[14rem] h-[50px] text-base items-center" />
              </div>
              <div class="flex flex-col">
                <span class="text-xs uppercase mb-2" :style="{ color: themeMode.darkMode ? 'white' : '#5e685e' }">Data de ida:</span>
                <DatePicker v-model="datePickerIda" showIcon iconDisplay="input" placeholder="DD/MM/YYYY" class="md:w-[14rem] h-[50px]"/>
              </div>
              <div class="flex flex-col">
                <span class="text-xs uppercase mb-2" :style="{ color: themeMode.darkMode ? 'white' : '#5e685e' }">Data de volta:</span>
